 ...
 ****************************************************/
 ...
 test('sendConfirmedDelivery - notification is null - feature flag is 1', async t => {
   const notificationId = null;
   const notificationPutCall = mockNotificationPutCall(notificationId);
   await fakeSetSubscription();

   await OSServiceWorker.sendConfirmedDelivery({ id: notificationId, rr: 1 });
   t.false(notificationPutCall.isDone());
 });

 test('sendConfirmedDelivery - notification is valid - feature flag is 1', async t => {
  const notificationId = Random.getRandomUuid();
  const notificationPutCall = mockNotificationPutCall(notificationId);
  await fakeSetSubscription();
 ...
 test('sendConfirmedDelivery - notification is valid - feature flag is 0', async t => {
  const notificationId = Random.getRandomUuid();
  const notificationPutCall = mockNotificationPutCall(notificationId);
  await fakeSetSubscription();

  await OSServiceWorker.sendConfirmedDelivery({ id: notificationId, rr: 0 });
  t.false(notificationPutCall.isDone());
 });
