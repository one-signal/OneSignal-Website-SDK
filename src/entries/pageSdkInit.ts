...
function oneSignalSdkInit() {
  ...
  const predefinedOneSignalPushes: OneSignalStubES6 | object[] | undefined | null = (window as any).OneSignal;

  (window as any).OneSignal = require('../OneSignal').default;

  ReplayCallsOnOneSignal.doReplay(predefinedOneSignalPushes);
}
